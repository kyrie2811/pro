            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    if (this.ruleForm.capchar.toLowerCase() === this.getCookie('captcha')) {
                        this.$axios.post('http://localhost:3000/api/useradmin/account/userlogin', this.ruleForm).then(res => {
                            if (res) {
                                // 保存token到本地
...
#login_box {
    height: 100%;
    background-image: url('https://img2.baidu.com/it/u=2256820927,3050079446&fm=253&fmt=auto&app=138&f=JPEG?w=800&h=500');
