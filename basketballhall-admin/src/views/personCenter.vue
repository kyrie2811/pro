...
export default {
    ...
    computed: {
        adminInfo: function () {
            return this.$store.state.adminStore.adminInfo
        }
    },
    created() {
        this.getAllmsg()
    ...
    methods: {
        // 获取信息
        getAllmsg() {
            console.log('this.$store.state.adminStore.adminInfo=>', this.$store.state.adminStore.adminInfo)
            const item = {
                email: this.$store.state.adminStore.adminInfo.email
            }
            this.$axios.post('http://localhost:3000/api/admin/allpersonmsg', item)
                .then(res => {
                    console.log('获取到的所有的信息res=>', res)
                    this.form = res.data.result
                    this.$store.dispatch('setAdminInfo', res.data.result)
                })
        ...
        editSite(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    // 调用接口，编辑个人中心
                    this.$axios.post('http://localhost:3000/api/admin/editmsg', {
        ...
        handleMusicPoster(res) {
            ...
            this.siteSrc = 'http://localhost:3000/api/admin/poster?img=' + this.form.poster
